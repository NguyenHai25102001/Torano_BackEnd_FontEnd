...
import {createCategory, editCategory, showCategory} from "../../components/dataApi";
import Swal from "sweetalert2";
import {FontAwesomeIcon} from "@fortawesome/react-fontawesome";
import {faPlus, faX} from "@fortawesome/free-solid-svg-icons";
import {ImageUrl} from "../../components/dataApi";
import axios from "axios";


function EditCategory({onClose,nameCategory,categoryId,categoryImage}) {
    ...
    useEffect(()=>{
        if(categoryImage){
            const path=ImageUrl+categoryImage;
            setShowFile(path)
        }
    },[categoryImage])
        ...
        <div className="sidebar-main">
            <div className={"category-create rounded shadow bg-body p-3 is-opened "}>
                <form onSubmit={handleCategory}>
                    <div className="row">
                        <div className="col-md-12 my-2 fs-5 fw-bolder text-center text-primary">Sửa danh mục</div>
                        <div className="col-md-12">
                            <label className='form-label' htmlFor='namCategory'>Tên danh mục</label>
                            <input type="text" id='namCategory' value={name} className='form-control' onChange={(e)=>setName(e.target.value)}/>
                        </div>
                        ...
                        <div className="col-md-12">
...

                            {
                                showFile ?<div className="w-100 position-relative add-img-category rounded"
                                    ...
                                    <img src={showFile} alt="" className='w-100 object-fit-cover'/>


                                    <div className="text-white btn-close-c position-absolute text-danger btn btn-close btn-danger"
                                         role={'button'} onClick={()=>{
                                        setShowFile('');
                                        setFile()
                                    }}>
