...
import {createChildCategory, editChildCategory, listSubcategory} from "../../components/dataApi";
import Swal from "sweetalert2";
function EditChildCategory({ onClose, listCategory, categoryId, subId, childName, childId}) {
    ...
    const handleSubmit=async(e)=>{
        ...
        try {
            const response=await editChildCategory(childCategoryId,data)
            console.log(response)
            if(response.data){
                Swal.fire({
                    position: "top-end",
                    icon: "success",
                    title:"Update Child Category thành công",
                ...
                for (const key in response.error) {
                    if (e.error.hasOwnProperty(key) && Array.isArray(response.error[key]) && response.error[key].length > 0) {
            ...
            <div className={"category-create rounded shadow bg-body p-3 is-opened "}>
                <form onSubmit={handleSubmit}>
                    <div className="row">
                        ...
                        <div className="col-md-12 mt-3 pe-none">
