...
import routesAdmin from "../route";
import {BrowserRouter as Router, Route, Routes} from "react-router-dom";
import navAdmin from "../route/navAdmin";
import $ from 'jquery'
...
function DefaultLayout({component: Component, ...rest}) {
    ...
    const {isLoginIn, logout} = useAuth();

    const [search, setSearch] = useState('')

    React.useEffect(() => {
        const handleResize = () => {
            // Kích thước màn hình nhỏ hơn 700
    ...
    const handleSearch = (e) => {
        ...
        if (search !== '') {
            window.location.href = `/admin/product/index?search=${search}`;
                        ...
                        </Link>
                        {
                            navAdmin.map((item, index) => (
                                <div key={index}>
                                    ...
                                    {/* Heading */}
                                    <div className="sidebar-heading">{item.section}</div>
                                    {
                                        item.links?.map((itemLink, idx) => (
                                            <li className="nav-item" key={idx}>
                                                <NavLink to={itemLink.path}
